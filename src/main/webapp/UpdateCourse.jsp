...
<%
	...
	if (session.getAttribute("UpdateCode") != null)
    	code = session.getAttribute("UpdateCode").toString();
	if (session.getAttribute("UpdateSubject") != null)
    	fetchedName = session.getAttribute("UpdateSubject").toString();
	if (session.getAttribute("UpdateCourseDno") != null)
    	fetchedDno = session.getAttribute("UpdateCourseDno").toString();
	if (session.getAttribute("UpdateCourseYear") != null)
		fetchedyear = session.getAttribute("UpdateCourseYear").toString();
	if (session.getAttribute("UpdateCourseSem") != null)
		fetchedsem = session.getAttribute("UpdateCourseSem").toString();
	if (session.getAttribute("UpdateCredits") != null)   
		fetchedcredits = session.getAttribute("UpdateCredits").toString();
%>
...
%>
  <div class="error-msg msg-box">You are not authorized to Update Course details for the selected department.</div>
<% } else if ("true".equals(success)) { %>
  <div class="success-msg msg-box">Course details Updated successfully.</div>
...
<% } %>
  ...
  <form id="fetchcourseForm" class="update-form" method="post" action="FetchCourseUpdate" novalidate>
  ...
  <form id="updatecourseForm" class="update-form" action="UpdateCourse" method="post" novalidate>
  ...
  <% } %>
    ...
    <button type="submit" id="updateBtn" <%= code.isEmpty() ? "disabled" : "" %>>Update</button>

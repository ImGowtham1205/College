...
<nav class="navbar">
 ...
 <!-- It Checks That Any Subject Assigned For That Particular If Assigend Then Display The Assigned Subject Else It Display "No Subject Assigned" Message-->
  ...
  <% } else { %>
      <h3>Click Subject Name To Update Attendance To Selected Student:</h3>
        ...
        for (FetchAssignStaff sub : list3rdyear) {
            ...
            int sem=sub.getSem();
        %>
       		<div class="subject-card" onclick="updateAttendance('<%= subject %>', '<%= code %>' ,'<%= year %>')">
  			<%= subject %><br>
    		Course Code : <%= code %><br>
    		Staff Name : <%= sname %><br>
    		Staff Id : <%= id %><br>
...
<!-- This Is Function Is Used For To Update Attendance For Student-->
  	 <script>
  function updateAttendance(subjectName, code,year) {

...
<form id="hodForm" class="hod-form" method="post" action="UpdateAssignedStaff" novalidate>
  ...
  %>
          ...
          <% for (FetchAssignStaff sub : semlist) {
              ...
              int sem = sub.getSem();
          %>
              <div class="subject-card" onclick="updateSubject('<%= subject %>', '<%= code %>' ,'<%= sname %>', '<%= id %>' ,'<%= sem %>')">
                  <%= subject %><br>
                  Course Code : <%=code %><br>
                  Staff Name : <%= sname %><br>
                  Staff Id : <%= id %><br>
  ...
  <script src="jscodes/UpdateSubject.js"></script> 	 
